#!/usr/bin/env python3
"""
Script de conversion Markdown vers Word (DOCX) - Version corrigée
Convertit le plan de maintenance Cesizen du format MD vers DOCX avec formatage gras fonctionnel
"""
...
class MarkdownToDocxConverter:
    ...
    def add_formatted_text(self, paragraph, text):
        """Ajoute du texte formaté (gras) à un paragraphe"""
        if '**' not in text:
            paragraph.add_run(text)
            return
        
        # Traitement manuel du gras avec regex
        import re
        
        # Remplacer tous les **texte** par du formatage gras
        pattern = r'\*\*(.*?)\*\*'
        last_end = 0
        
        for match in re.finditer(pattern, text):
            # Ajouter le texte avant le match (normal)
            if match.start() > last_end:
                paragraph.add_run(text[last_end:match.start()])
            
            # Ajouter le texte gras
            bold_text = match.group(1)
            run = paragraph.add_run(bold_text)
            run.bold = True
            run.font.bold = True  # Double application pour garantie
    ...
    def process_table(self, table_lines):
        """Traite les lignes d'un tableau markdown avec formatage gras"""
        ...
        # Ajoute les en-têtes avec formatage
        header_row = table.rows[0]
        for i, header in enumerate(headers):
            cell_para = header_row.cells[i].paragraphs[0]
            cell_para.clear()
            self.add_formatted_text(cell_para, header)
            # Forcer le gras pour tous les en-têtes
            ...
            if '|' in line:
                ...
                if len(cells) == len(headers):
                    ...
                    for i, cell_content in enumerate(cells):
                        ...
                        cell_para.clear()
                        self.add_formatted_text(cell_para, cell_content)
    ...
    def convert_file(self, input_file, output_file):
        ...
        for line in lines:
            ...
            if line_type == 'empty':
                ...
                self.add_formatted_text(para, content)
